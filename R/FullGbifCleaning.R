...
# This is by Jeremy or Brian, taken from another project
...
#' Full GBIF filtering - it's merely a wraper of all other functions
...
#' @export
FullGbifCleaning <- function(points, species="species", lon="decimalLongitude", lat="decimalLatitude") {
  print("Removing coordinates with no decimals.")
  try(cleaned_points <- RemoveNoDecimal(points, lon, lat))
  print("Removing centroids.")
  try(cleaned_points <- RemoveCentroids(cleaned_points, lon, lat))
  print("Removing duplicates")
  try(cleaned_points <- RemoveDuplicates(cleaned_points, lon, lat))
  print("Removing points on the sea.")
  try(cleaned_points <- RemoveSeaPoints(cleaned_points, lon, lat))
  print("Removing wrong countries.")
  try(cleaned_points <- RemoveWrongCountries(cleaned_points, lon, lat))
  print("Removing points close to big herbaria.")
  try(cleaned_points <- RemoveHerbariaLocalities(cleaned_points, lon, lat))
  print("Removing points classified as crops of invasive.")
  try(cleaned_points <- RemoveCropsAndInvasive(cleaned_points, species))
  print("Removing points with lon and lat equal to 0.")
  try(cleaned_points <- RemoveZeros(cleaned_points, lon, lat))
  print("Removing outliers")
  try(cleaned_points <- RemoveOutliers(cleaned_points, species, lon, lat))
...
#' Removes invasive and crop species. Names of species must be given without author name and following the GBIF taxonomic backbone
...
#' @param species The name of the column in the data.frame with the names of species
#' @export
RemoveCropsAndInvasive <- function(points, species="species") {
  npoints_start <- nrow(points)
  tmp_points = points
  colnames(tmp_points)[colnames(tmp_points)==species] <- "species"
  invasive <- readRDS("R/invasive_taxized_gbif.Rdata")
  crops <- readRDS("R/crops_taxized_gbif.Rdata")
  invasive_and_crops <- unique(c(invasive, crops))
  if(any(tmp_points$species %in% invasive_and_crops)) {
    points <- points[-which(tmp_points$species %in% invasive_and_crops), ]
  }
  npoints_end <- nrow(points)
  print(paste0(npoints_start - npoints_end, " points removed."))
  return(points)
